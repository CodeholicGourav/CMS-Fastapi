...
class FrontendEmail(BaseEmail):
    def sendEmailVerificationToken(self, user: BackendUser):
        ...
        verification_link = f"{os.getenv('SERVER_URL')}/{EMAIL_VERIFY_URL}?token={user.verification_token}"  # Include the verification token in the link
    ...
    def sendForgetPasswordToken(self, user: BackendUser):
        ...
        verification_link = f"{os.getenv('SERVER_URL')}/{CREATE_PASSWORD_URL}?token={user.verification_token}"  # Include the verification token in the link
