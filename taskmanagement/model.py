...
class Project(Base):
    ...
    puid = Column(String(50), index=True, unique=True)
    ...
    columns = relationship("CustomColumn", back_populates="project")
...
class Task(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    tuid = Column(String(50), index=True, unique=True)
    ...
    estimate_hours = Column(Integer, comment="in hours")
    ...
    creator = relationship("FrontendUser", foreign_keys=created_by)
    parent = relationship("Task", foreign_keys=parent_id)
    ...
    assigned_to = relationship("UserTask", back_populates="task")
...
class TaskGroup(Base):
    ...
    guid = Column(String(50), index=True, unique=True)
    title = Column(
        String(50),
    )
    project_id = Column(Integer, ForeignKey("projects.id"))
    ...
    def __repr__(self):
        return "CustomColumnExpected(" f"guid={self.guid}, " f"value='{self.title}'" ")"
...
def create_proj_permissions():
    ...
    try:
        ...
        sql.commit()
        return {"message": "Project Permissions created successfully"}
...
class ProjectUserPermission(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    user_id = Column(Integer, ForeignKey("frontendusers.id"), nullable=True)
    project_id = Column(Integer, ForeignKey("projects.id"), nullable=True)
    permission_id = Column(
        Integer, ForeignKey("project_permissions.id"), nullable=False
    )

    user = relationship("FrontendUser", foreign_keys=user_id)
    project = relationship("Project", foreign_keys=project_id)
    permission = relationship("ProjectPermission", foreign_keys=permission_id)
...
class UserTask(Base):
    ...
    user = relationship("FrontendUser", foreign_keys=user_id)
    assigned_by = relationship("FrontendUser", foreign_keys=created_by)
...
class CustomColumn(Base):
    ...
    id = Column(Integer, primary_key=True)
    cuid = Column(String(50), index=True, unique=True)
    project_id = Column(Integer, ForeignKey("projects.id"))
    column_name = Column(String(50), unique=True)
    type = Column(String(50))
    created_by = Column(Integer, ForeignKey("frontendusers.id"))
    is_deleted = Column(Boolean, default=False)
    deleted_by = Column(Integer, ForeignKey("frontendusers.id"))
    created_at = Column(DateTime, default=datetime.utcnow)
    updated_at = Column(DateTime, default=datetime.utcnow)

    creator = relationship("FrontendUser", foreign_keys=created_by)
    project = relationship("Project", foreign_keys=project_id)
    values = relationship("CustomColumnExpected", back_populates="column")
...
class CustomColumnExpected(Base):
    ...
    id = Column(Integer, primary_key=True)
    vuid = Column(String(50), index=True, unique=True)
    value = Column(
        String(50),
    )
    column_id = Column(Integer, ForeignKey("custom_columns.id"))
    is_deleted = Column(Boolean, default=False)
    created_at = Column(DateTime, default=datetime.utcnow)
    updated_at = Column(DateTime, default=datetime.utcnow)

    column = relationship("CustomColumn", foreign_keys=column_id)
...
class CustomColumnAssigned(Base):
    ...
    id = Column(Integer, primary_key=True)
    value_id = Column(Integer, ForeignKey("custom_column_expected_values.id"))
    column_id = Column(Integer, ForeignKey("custom_columns.id"))
    task_id = Column(Integer, ForeignKey("tasks.id"))
    created_at = Column(DateTime, default=datetime.utcnow)
    updated_at = Column(DateTime, default=datetime.utcnow)
    ...
    value = relationship("CustomColumnExpected", foreign_keys=value_id)
...
class Comments(Base):
    ...
    id = Column(Integer, primary_key=True)
    cuid = Column(String(50), unique=True)
    ...
    def __repr__(self):
        ...
        """
        return f"Comment(id={self.id}, cuid='{self.cuid}', comment='{self.comment}')"
